...
Open **Zekai kotaları** in the existing admin menu or visit `/yazi-atolyesi/admin/budgets`.
...
Image polling uses `FAL_POLL_INTERVAL_SECONDS=3` and `FAL_POLL_LIMIT=40` (about two minutes of polling, plus HTTP time). `IMAGE_GENERATION_TIMEOUT_SECONDS=360` controls the overall application deadline and PHP execution allowance; the editor waits an additional 30 seconds. Poll exhaustion returns JSON with HTTP 504, checks and request ID; a completed response without an image returns HTTP 502. Neither case silently returns an empty 200. Web-server/proxy timeouts are separate from these application settings. `php artisan test --filter=ImagePollingTest` verifies the polling paths with a mocked provider and no real waits or charges.
