...
class ImportedWriterWorkspaceTest extends WriterTestCase
{
    ...
    public function test_ai_html_round_trip_keeps_formatting_and_excludes_it_from_classification(): void
    {
        $book = $this->book();
        $this->catalog();
        $book->document = \App\Writer\Services\ManuscriptHtml::replacement('<h1>Opening</h1><p>Mara <strong>waited</strong> by the <em>gate</em>.</p>', 'html');
        $book->save();
        ...
        $this->postJson('/yazi-atolyesi/api/books/'.$book->id.'/chat', ['request_id' => (string) Str::uuid(), 'message' => 'Revise', 'model' => 'test/writer', 'history' => '0'])->assertOk();
        $sent = Http::recorded();
        ...
        $this->postJson('/yazi-atolyesi/api/books/'.$book->id.'/proposals/'.$proposal->id, ['accept' => [0]])->assertOk();
        $this->assertSame($html, str_replace("\n", '', \App\Writer\Services\ManuscriptHtml::html($book->fresh()->document)));
    }
    ...
    {
        $doc = \App\Writer\Services\ManuscriptHtml::replacement('<p><strong>😀 left </strong><em>old</em><code> right</code></p>');
        $scope = ['from_block' => 0, 'to_block' => 0, 'from_offset' => 8, 'to_offset' => 11, 'text' => 'old'];
        $this->assertSame('<p><em>old</em></p>', \App\Writer\Services\ManuscriptHtml::html(\App\Writer\Services\SelectionEdit::document($doc, $scope)));
        $result = \App\Writer\Services\SelectionEdit::apply($doc, $scope, '<p><em>new</em></p>');
        $this->assertSame('<p><strong>😀 left </strong><em>new</em><code> right</code></p>', \App\Writer\Services\ManuscriptHtml::html($result));
    }

    public function test_html_import_rejects_interactive_markup_and_ignores_attributes(): void
    {
        $doc = \App\Writer\Services\ManuscriptHtml::replacement('<p onclick="bad()"><strong style="color:red">Safe</strong><br>text</p><hr>');
        $this->assertSame("<p><strong>Safe</strong><br>text</p>\n<hr>", \App\Writer\Services\ManuscriptHtml::html($doc));
        $this->expectException(\Illuminate\Validation\ValidationException::class);
        \App\Writer\Services\ManuscriptHtml::replacement('<p>Text<button>Click</button></p>');
    }
    ...
    public function test_books_cannot_be_accessed_by_another_account(): void
    {
        ...
        $this->getJson('/yazi-atolyesi/api/books/'.$book->id)->assertNotFound();
        $this->patchJson('/yazi-atolyesi/api/books/'.$book->id, ['revision' => 1, 'title' => 'Stolen'])->assertNotFound();
        $this->postJson('/yazi-atolyesi/api/books/'.$book->id.'/chat', [])->assertNotFound();
    }
    ...
    public function test_revision_details_are_scoped_to_book_and_owner(): void
    {
        ...
        $url = '/yazi-atolyesi/api/books/'.$book->id.'/revisions/'.$revision->id;
        $this->getJson($url)->assertOk()->assertJsonPath('snapshot.title', 'The Orchard');
        $second = $this->book(User::find($book->user_id));
        $this->getJson('/yazi-atolyesi/api/books/'.$second->id.'/revisions/'.$revision->id)->assertNotFound();
        $this->actingAs(User::factory()->create());
    ...
    public function test_selection_edit_preserves_unselected_text_and_formatting(): void
    {
        ...
        $this->postJson('/yazi-atolyesi/api/books/'.$book->id.'/chat', ['request_id' => (string) Str::uuid(), 'message' => 'Change the name', 'model' => 'test/writer', 'history' => '0', 'selection' => ['revision' => 1, 'from_block' => 0, 'to_block' => 0, 'from_offset' => 3, 'to_offset' => 7, 'text' => 'Mara']])->assertOk();
        $proposal = $book->proposals()->first();
        $this->postJson('/yazi-atolyesi/api/books/'.$book->id.'/proposals/'.$proposal->id, ['accept' => [0]])->assertOk();
    ...
    public function test_selection_scope_rejects_whole_paragraph_ai_changes(): void
    {
        ...
        $this->postJson('/yazi-atolyesi/api/books/'.$book->id.'/chat', ['request_id' => (string) Str::uuid(), 'message' => 'Rename', 'model' => 'test/writer', 'history' => '0', 'selection' => ['revision' => 1, 'from_block' => 0, 'to_block' => 0, 'from_offset' => 0, 'to_offset' => 4, 'text' => 'Mara']])->assertUnprocessable();
    ...
    public function test_stale_selection_is_rejected_before_any_paid_call(): void
    {
        ...
        $this->catalog();
        Http::fake();
        $this->postJson('/yazi-atolyesi/api/books/'.$book->id.'/chat', ['request_id' => (string) Str::uuid(), 'message' => 'Rename', 'model' => 'test/writer', 'history' => '0', 'selection' => ['revision' => 0, 'from_block' => 0, 'to_block' => 0, 'from_offset' => 0, 'to_offset' => 4, 'text' => 'Mara']])->assertStatus(409);
    ...
    public function test_selection_can_cross_paragraphs_without_touching_outside_blocks(): void
    {
        ...
        $saved = \App\Writer\Services\SelectionEdit::apply($doc, $scope, "revised\npassage");
    ...
    public function test_llm_payloads_are_recorded_and_scoped_without_authentication_headers(): void
    {
        ...
        $this->getJson('/yazi-atolyesi/api/books/'.$book->id.'/llm-log')->assertOk()->assertJsonCount(1, 'data')->assertJsonMissingPath('data.0.request_payload');
        $other = $this->book(User::find($book->user_id));
        $this->getJson('/yazi-atolyesi/api/books/'.$other->id.'/llm-log/'.$call->id)->assertNotFound();
        $this->actingAs(User::factory()->create());
    ...
    public function test_deleted_messages_leave_history_but_keep_request_deduplication(): void
    {
        ...
        $other = $this->book(User::find($book->user_id));
        $this->deleteJson('/yazi-atolyesi/api/books/'.$other->id.'/messages/'.$message->id)->assertNotFound();
        $this->deleteJson('/yazi-atolyesi/api/books/'.$book->id.'/messages/'.$message->id)->assertOk();
        $this->getJson('/yazi-atolyesi/api/books/'.$book->id)->assertJsonCount(0, 'messages');
        Http::fake();
        $this->postJson('/yazi-atolyesi/api/books/'.$book->id.'/chat', ['request_id' => $requestId, 'message' => 'Retry', 'model' => 'test/writer', 'history' => 'all'])->assertStatus(409);
        Http::assertNothingSent();
    ...
    public function test_formatted_manuscript_whitespace_is_preserved_and_nested_documents_rejected(): void
    {
        ...
        $this->patchJson('/yazi-atolyesi/api/books/'.$book->id, ['revision' => 1, 'document' => $doc])->assertOk();
        $this->assertSame('Mara waited by the gate.', $book->fresh()->manuscript);
        $this->assertSame($doc, $book->fresh()->document);
        $this->patchJson('/yazi-atolyesi/api/books/'.$book->id, ['revision' => 2, 'document' => ['type' => 'doc', 'content' => [$doc]]])->assertStatus(422);
    }
    ...
    public function test_codex_aliases_types_and_restore_keep_reference_ids(): void
    {
        ...
        $this->assertDatabaseHas('writer_codex_entries', ['id' => $entry['id'], 'content' => 'A traveler.']);
    }

    public function test_names_use_full_countries_and_filter_local_datasets(): void
    {
        $this->actingAs(User::factory()->create());
        $countries = $this->getJson('/yazi-atolyesi/api/countries')->assertOk()->json();
        $this->assertSame('United States', collect($countries)->firstWhere('code', 'US')['name']);
        $this->getJson('/yazi-atolyesi/api/names?country=US&gender=Female&q=Maria')->assertOk()->assertJsonPath('first.0.name', 'Maria');
        $this->getJson('/yazi-atolyesi/api/names?country=US&last_country=BR&random=1')->assertOk()->assertJsonCount(10, 'results');
        $this->getJson('/yazi-atolyesi/api/names?country=../')->assertStatus(422);
    }
    ...
    public function test_catalog_only_refreshes_once_per_login_flag(): void
    {
        ...
        $this->withSession(['refresh_model_catalog' => true])->getJson('/yazi-atolyesi/api/models')->assertOk();
        $this->getJson('/yazi-atolyesi/api/models')->assertOk();
        $this->getJson('/yazi-atolyesi/api/models')->assertOk();
    ...
    public function test_two_stage_chat_uses_light_classification_and_zero_history_and_only_proposes_changes(): void
    {
        ...
        $this->postJson('/yazi-atolyesi/api/books/'.$book->id.'/chat', ['request_id' => (string) Str::uuid(), 'message' => 'Edit the opening.', 'model' => 'test/writer', 'history' => '0'])->assertOk();
        $sent = Http::recorded();
        ...
        $this->postJson('/yazi-atolyesi/api/books/'.$book->id.'/proposals/'.$proposal->id, ['accept' => [0]])->assertStatus(409);
    ...
    public function test_stale_proposals_cannot_overwrite_newer_work_but_can_be_rejected(): void
    {
        ...
        $this->postJson('/yazi-atolyesi/api/books/'.$book->id.'/proposals/'.$proposal->id, ['accept' => [0]])->assertStatus(409);
        $this->postJson('/yazi-atolyesi/api/books/'.$book->id.'/proposals/'.$proposal->id, ['accept' => []])->assertOk();
        $this->assertSame('Mara waited by the gate.', $book->fresh()->manuscript);
    }
    ...
    public function test_missing_person_names_stops_before_second_paid_call(): void
    {
        ...
        $this->postJson('/yazi-atolyesi/api/books/'.$book->id.'/chat', ['request_id' => (string) Str::uuid(), 'message' => 'Create ten people', 'model' => 'test/writer', 'history' => 'all'])->assertOk()->assertJsonPath('needs_names', true);
        Http::assertSentCount(1);
        $this->assertSame(0, $book->proposals()->count());
    ...
    public function test_place_suggestions_are_saved_for_selection_without_codex_writes(): void
    {
        ...
        $this->postJson('/yazi-atolyesi/api/books/'.$book->id.'/chat', ['request_id' => (string) Str::uuid(), 'message' => 'Suggest coastal names', 'action' => 'places', 'country' => 'United States', 'model' => 'test/writer', 'history' => '0'])->assertOk();
    ...
    public function test_partial_batch_approval_only_writes_selected_entries_and_retries_do_not_duplicate(): void
    {
        ...
        $this->postJson('/yazi-atolyesi/api/books/'.$book->id.'/proposals/'.$proposal->id, ['accept' => [1]])->assertOk();
        $this->assertSame(['Lantern'], $book->entries()->pluck('name')->all());
        $this->getJson('/yazi-atolyesi/api/books/'.$book->id)->assertOk()->assertJsonPath('proposals.0.status', 'approved')->assertJsonPath('proposals.0.decisions', [1]);
        $this->postJson('/yazi-atolyesi/api/books/'.$book->id.'/proposals/'.$proposal->id, ['accept' => [1]])->assertStatus(409);
    ...
    public function test_model_failure_preserves_document_and_reused_request_id_does_not_call_again(): void
    {
        ...
        $this->postJson('/yazi-atolyesi/api/books/'.$book->id.'/chat', $request)->assertUnprocessable();
        $this->postJson('/yazi-atolyesi/api/books/'.$book->id.'/chat', $request)->assertStatus(409);
        Http::assertSentCount(1);
        $this->assertSame('Mara waited by the gate.', $book->fresh()->manuscript);
    }
    ...
    public function test_timed_out_chat_releases_lock_and_next_send_succeeds_without_refresh(): void
    {
        ...
        $this->postJson('/yazi-atolyesi/api/books/'.$book->id.'/chat', $request + ['request_id' => (string) Str::uuid()])->assertUnprocessable()->assertJsonValidationErrors('ai');
        $this->postJson('/yazi-atolyesi/api/books/'.$book->id.'/chat', $request + ['request_id' => (string) Str::uuid()])->assertOk();
    ...
    public function test_abandoned_messages_are_failed_only_after_the_request_lock_is_released(): void
    {
        ...
        try {
            $this->getJson('/yazi-atolyesi/api/books/'.$book->id)->assertOk()->assertJsonPath('messages.0.status', 'pending');
        } finally {
            $lock->release();
        }
        $this->getJson('/yazi-atolyesi/api/books/'.$book->id)->assertOk()->assertJsonPath('messages.0.status', 'failed');
        $this->deleteJson('/yazi-atolyesi/api/books/'.$book->id.'/messages/'.$message->id)->assertOk();
        $this->assertSoftDeleted('writer_chat_messages', ['id' => $message->id]);
        $this->getJson('/yazi-atolyesi/api/books/'.$book->id)->assertOk()->assertJsonCount(0, 'messages');
    }

    public function test_chat_process_lock_prevents_parallel_requests_and_releases_explicitly(): void
    {
        $book = $this->book();
        $first = new \App\Writer\Services\BookChatLock;
        $second = new \App\Writer\Services\BookChatLock;
        $this->assertTrue($first->acquire($book->id));
        try {
            $this->assertFalse($second->acquire($book->id));
            $this->postJson('/yazi-atolyesi/api/books/'.$book->id.'/chat', ['request_id' => (string) Str::uuid(), 'message' => 'Hello', 'model' => 'test/writer', 'history' => '0'])->assertStatus(409);
        } finally {
            $first->release();
        }
    ...
    public function test_batch_and_image_generation_models_are_rejected_before_payment(): void
    {
        ...
        foreach ([['name' => 'Writer (BATCH)'], ['architecture' => ['output_modalities' => ['text', 'image']]]] as $unsupported) {
            ...
            $this->postJson('/yazi-atolyesi/api/books/'.$book->id.'/chat', ['request_id' => (string) Str::uuid(), 'message' => 'Edit', 'model' => 'test/writer', 'history' => 'all'])->assertUnprocessable()->assertJsonValidationErrors('model');
        }
        Http::assertNothingSent();
    ...
    public function test_optional_search_fees_and_pricing_tiers_allow_text_requests(): void
    {
        ...
        $this->postJson('/yazi-atolyesi/api/books/'.$book->id.'/chat', ['request_id' => (string) Str::uuid(), 'message' => 'Hello', 'model' => 'test/writer', 'history' => '0'])->assertOk();
        Http::assertSentCount(2);
    ...
    public function test_text_fees_and_tiers_are_reserved_before_payment(): void
    {
        ...
        Cache::forever('openrouter.catalog', ['data' => [$model], 'refreshed_at' => null]);
        Http::fake();
        $this->postJson('/yazi-atolyesi/api/books/'.$book->id.'/chat', ['request_id' => (string) Str::uuid(), 'message' => 'Hello', 'model' => 'test/writer', 'history' => '0'])->assertUnprocessable();
        Http::assertNothingSent();
    }
    ...
    public function test_context_overflow_is_rejected_before_any_paid_request(): void
    {
        ...
        Cache::forever('openrouter.catalog', ['data' => [$model], 'refreshed_at' => null]);
        Http::fake();
        $this->postJson('/yazi-atolyesi/api/books/'.$book->id.'/chat', ['request_id' => (string) Str::uuid(), 'message' => 'Edit', 'model' => 'test/writer', 'history' => 'all'])->assertStatus(422);
        Http::assertNothingSent();
        $this->assertSame(0, AiCall::count());
    }
    ...
    public function test_scan_can_extract_existing_person_names_without_generating_them(): void
    {
        ...
        $this->postJson('/yazi-atolyesi/api/books/'.$book->id.'/chat', ['request_id' => (string) Str::uuid(), 'message' => 'Scan the story', 'action' => 'scan', 'model' => 'test/writer', 'history' => 'all'])->assertOk();
        $this->assertSame(0, $book->entries()->count());
    ...
    public function test_cursor_focus_and_selection_context_have_different_manuscript_scopes(): void
    {
        ...
        foreach ([false, true] as $selected) {
            ...
            $this->postJson('/yazi-atolyesi/api/books/'.$book->id.'/chat', $request)->assertOk();
            $sent = Http::recorded();
    ...
    public function test_context_window_preserves_unicode_and_expands_partial_sentences(): void
    {
        ...
        $window = \App\Writer\Services\ManuscriptContext::window($doc, ['block' => 0, 'offset' => 12], ['block' => 0, 'offset' => 16], 1);
    ...
    public function test_stale_cursor_is_rejected_before_llm_calls(): void
    {
        $book = $this->book()->fresh();
        Http::fake();
        $this->postJson('/yazi-atolyesi/api/books/'.$book->id.'/chat', ['request_id' => (string) Str::uuid(), 'message' => 'Review here', 'model' => 'test/writer', 'history' => '0', 'cursor' => ['revision' => $book->revision + 1, 'block' => 0, 'offset' => 0]])->assertStatus(409);
        Http::assertNothingSent();
    }
    ...
    public function test_large_prompt_warning_precedes_payment_and_can_be_forced_or_snoozed(): void
    {
        ...
        $response = $this->postJson('/yazi-atolyesi/api/books/'.$book->id.'/chat', $payload)->assertOk()->assertJsonPath('large_prompt_warning', true);
        $this->assertGreaterThan(30000, $response->json('word_count'));
        Http::assertNothingSent();
        $this->assertSame(0, AiCall::count());
        $this->assertSame(0, $book->messages()->count());
        $this->postJson('/yazi-atolyesi/api/books/'.$book->id.'/chat', $payload + ['force_large_prompt' => true, 'disable_large_prompt_warning' => true])->assertOk()->assertJsonPath('ok', true);
        Http::assertSentCount(2);
        $payload['request_id'] = (string) Str::uuid();
        $this->travel(59)->minutes();
        $this->postJson('/yazi-atolyesi/api/books/'.$book->id.'/chat', $payload)->assertOk()->assertJsonPath('ok', true);
        Http::assertSentCount(4);
        $this->travel(2)->minutes();
        $payload['request_id'] = (string) Str::uuid();
        $this->postJson('/yazi-atolyesi/api/books/'.$book->id.'/chat', $payload)->assertOk()->assertJsonPath('large_prompt_warning', true);
        Http::assertSentCount(4);
        $this->travelBack();
    }
