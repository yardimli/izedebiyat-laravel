...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const openComposer = (postId = "", name = "") => {
        ...
        if (context)
            context.textContent = name ? `${name} kullanıcısına yanıt` : "Yanıt yazıyorsunuz";
        composer.scrollIntoView({ behavior: "smooth", block: "center" });
        window.setTimeout(() => composer.querySelector("form")._forumQuill?.focus(), 350);
    };

    document
        .querySelectorAll("[data-reply-open]")
    ...
    document
        .querySelectorAll("[data-reply-close]")
        .forEach((button) =>
            button.addEventListener("click", () => composer.classList.remove("is-open")),
        );
});
