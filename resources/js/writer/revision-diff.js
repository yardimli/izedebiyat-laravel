...
export function manuscript(doc) {
    function inline(node) {
        ...
        let text =
            node.text?.replace(/([\\*_`])/g, "\\$1") ?? (node.content || []).map(inline).join("");
    ...
    return (doc?.content || [])
        .map((node) =>
            node.type === "horizontal_rule"
                ? "---"
                : (node.type === "heading" ? "#".repeat(node.attrs?.level || 1) + " " : "") +
                  inline(node),
        )
        .join("\n");
}
...
export function revisionDiff(before, after) {
    ...
    const compare = (name, oldText, newText) => {
        const patch = structuredPatch(name, name, oldText, newText, t("Revision"), t("Current"), {
            context: 3,
            timeout: 1500,
        });
        if (!patch)
            throw new Error(
                t("This revision is too large to compare interactively. No changes were made."),
            );
    ...
    compare(t("Manuscript"), manuscript(before.document), manuscript(after.document));
