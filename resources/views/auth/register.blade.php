    ...
    <form method="POST" action="{{ route('register') }}" class="auth-form">
        ...
        <div class="field-pair">
            @include('auth.partials.field', [
                'name' => 'name',
                'label' => __('auth-page.name'),
                'autocomplete' => 'name',
                'max' => 60,
            ])
            @include('auth.partials.field', [
                'name' => 'username',
                'label' => __('auth-page.username'),
        ...
        @include('auth.partials.field', [
            ...
            'label' => __('default.Confirm Password'),
            'type' => 'password',
            'autocomplete' => 'new-password',
            'min' => 8,
        ])
        <label class="check-label policy"><input type="checkbox" name="policy" value="1" required
                @checked(old('policy'))><span>{!! __('default.I Agree with', [
                    'terms_url' => route('frontend.legal'),
                    'privacy_url' => route('frontend.secrecy'),
                ]) !!}</span></label>
        <div class="captcha-wrap">
            <div class="g-recaptcha" data-sitekey="{{ config('services.recaptcha.site_key') }}"></div>
        </div>
        <button class="auth-submit" type="submit">{{ __('default.Create Account') }} <span
