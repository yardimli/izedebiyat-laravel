        ...
        @if ($activeTag)
            <header class="forum-tag-hero" style="--tag-color: {{ $activeTag->color }}">
                ...
                @if ($activeTag->description)
                    <small>{{ $activeTag->description }}</small>
                @endif
            </header>
        @endif
                        ...
                        <ul class="dropdown-menu forum-sort-menu">
                            @foreach ($sortNames as $key => $name)
                                <li><a class="dropdown-item {{ $sort === $key ? 'active' : '' }}"
                                        href="{{ request()->fullUrlWithQuery(['sort' => $key, 'page' => null]) }}">{{ $name }}</a>
                                </li>
                            @endforeach
                        </ul>
                    </div>
                    <a href="{{ request()->fullUrl() }}" class="forum-icon-button" title="Yenile"><i
                ...
                <div class="forum-discussion-list">
                    @forelse($discussions as $discussion)
                        ...
                        <article class="forum-discussion-row">
                            <a href="{{ route('forum.profile', $activityUser) }}"
                                aria-label="{{ $activityUser->name }} profili">
                            ...
                            <div class="forum-discussion-summary">
                                <a class="forum-discussion-title" href="{{ route('forum.show', $discussion) }}">
                                    @if ($discussion->is_pinned)
                                        <i class="bi bi-pin-angle-fill" title="Sabitlendi"></i>
                                    @endif
                                    {{ $discussion->title }}
                                </a>
                                <div class="forum-discussion-meta">
                                    ...
                                    @endif
                                    <time
                                        datetime="{{ optional($discussion->last_post_at)->toIso8601String() }}">{{ optional($discussion->last_post_at)->diffForHumans() }}</time>
                                </div>
                            </div>
                            <div class="forum-discussion-stats">
                                <a class="forum-tag-pill" style="--tag-color: {{ $discussion->tag->color }}"
                                    href="{{ route('forum.tag', $discussion->tag) }}">{{ $discussion->tag->name }}</a>
                                <span title="İleti sayısı"><i class="bi bi-chat-fill"></i>
                                    {{ number_format($discussion->posts_count) }}</span>
                            </div>
                        </article>
                    @empty
                        <div class="forum-empty">
                            ...
                            <a href="{{ route('forum.create') }}" class="forum-button forum-button-primary">Tartışma
                                Başlat</a>
                        </div>
                    @endforelse
                </div>
