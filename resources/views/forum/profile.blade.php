    ...
    <div class="forum-shell">
        <header class="forum-profile-hero">
            <div class="forum-container">
                ...
                <div>
                    <h1>{{ $forumUser->name }}</h1><span>{{ $forumUser->created_at->format('F Y') }} tarihinde
                        katıldı</span>
                </div>
            </div>
        </header>
        <div class="forum-container forum-profile-layout">
            <aside class="forum-profile-stats">
                <a href="{{ route('forum.index') }}"><i
                        class="bi bi-chat-dots"></i><span>İletiler</span><strong>{{ $posts->total() }}</strong></a>
                ...
                @forelse($posts as $post)
                    <article>
                        <div class="forum-profile-post-heading">
                            ...
                            <div><a
                                    href="{{ route('forum.show', $post->discussion) }}#post-{{ $post->id }}">{{ $post->discussion->title }}</a><time>{{ $post->created_at->diffForHumans() }}</time>
                            </div>
                            <span class="forum-tag-pill"
                                style="--tag-color: {{ $post->discussion->tag->color }}">{{ $post->discussion->tag->name }}</span>
                        </div>
                        <div class="forum-post-body forum-post-excerpt">{!! $post->body !!}</div>
                    </article>
                @empty
                    <div class="forum-empty">
                        <p>Bu üyenin henüz forum iletisi yok.</p>
                    </div>
                @endforelse
