...
@section('content')
    <div class="forum-shell">
        <div class="forum-container">
            @include('forum._nav')
            <div class="forum-tags-header"><span>Konulara göz atın</span>
                <h1>Forum Etiketleri</h1>
            </div>
                ...
                @foreach ($tags as $tag)
                    <a href="{{ route('forum.tag', $tag) }}" class="forum-tag-card" style="--tag-color: {{ $tag->color }}">
                        ...
                        <div>
                            ...
                            @if ($tag->discussions->first())
                                <small>{{ Str::limit($tag->discussions->first()->title, 48) }}</small>
                            @endif
                        </div>
                    </a>
                @endforeach
