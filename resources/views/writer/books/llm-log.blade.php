...
<main class="llm-log-page history-page">
    <header class="history-hero"><div><div class="eyebrow">{{ __('BOOK HISTORY') }}</div><h1>{{ __('All LLM actions') }}</h1><p>{{ $book->title }} {{ __('· Review usage and full prompts and responses for this book. API keys are never included in request logs.') }}</p></div><a class="history-button" href="{{ route('articles.edit', \App\Helpers\IdHasher::encode($book->id)) }}">{{ __('Back to manuscript') }}</a></header>
    <div class="history-stats">
        ...
        <section><strong>{{ $key === 'cost' ? '$'.\App\Writer\Support\Money::display($summary->$key) : ($summary->$key === null ? '—' : number_format($summary->$key)) }}</strong><small>{{ __($label) }}</small></section>
        @endforeach
    </div>
    ...
    <form class="history-filters" method="get">
        ...
        <button class="primary">{{ __('Apply') }}</button><a class="history-button" href="{{ route('writer.books.llm-log', $book) }}">{{ __('Reset') }}</a>
    </form>
    ...
    <article class="history-call">
        ...
        <div class="history-call-stats">@foreach(['prompt_tokens'=>__('Prompt tokens'),'completion_tokens'=>__('Completion tokens'),'total_tokens'=>__('Total tokens'),'cost'=>__('Returned cost')] as $key=>$label)<div><small>{{ __($label) }}</small><strong>{{ $call->$key === null ? '—' : ($key === 'cost' ? '$'.\App\Writer\Support\Money::display($call->$key) : number_format($call->$key)) }}</strong></div>@endforeach</div>
        <div class="history-call-meta"><div><small>{{ __('API key source') }}</small>{{ $call->funding === 'personal' ? __('Personal OpenRouter key') : __('Demo allowance') }}</div><div><small>{{ __('Book') }}</small><a href="{{ route('articles.edit', \App\Helpers\IdHasher::encode($book->id)) }}">{{ $book->title }}</a></div><div><small>{{ __('Response') }}</small>{{ $call->response_status ? 'HTTP '.$call->response_status : __('Not received') }}</div><div><small>{{ __('Payload') }}</small>{{ $call->has_request ? __('Request recorded') : __('No recorded request') }} · {{ $call->has_response ? __('Response recorded') : __('No recorded response') }}</div></div>
        <footer><a class="history-button history-open" href="{{ route('writer.books.llm-call', [$book, $call->id]) }}">{{ __('View full prompt & response') }}</a></footer>
    </article>
