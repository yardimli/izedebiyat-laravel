...
<main class="library">
    ...
    <nav class="library-filters" aria-label="{{ __('Filter books') }}">
        ...
        <a href="{{ route('articles.index', ['filter' => $value]) }}" @if ($filter === $value) aria-current="page" @endif>{{ __($label) }}</a>
        @endforeach
    </nav>
    <h2 class="section-heading">{{ ['active' => __('On your desk'), 'archived' => __('Archived books'), 'deleted' => __('Recently deleted')][$filter] }} <span>{{ $books->total() }} {{ __('manuscripts') }}</span></h2>
    <div class="book-grid">
    @forelse ($books as $book)
        <article class="book-card {{ $book->trashed() || $book->archived ? 'book-muted' : '' }}">
            ...
            @else<a href="{{ route('articles.edit', \App\Helpers\IdHasher::encode($book->id)) }}">{{ __('Open manuscript ↗') }}</a>@endif</div>
            @unless ($book->trashed())
