...
<label>{{ __('Category') }}<select name="category_id"><option value="">{{ __('Choose a category') }}</option>@foreach($categories as $category)<option value="{{ $category->id }}">{{ $category->parentCategory ? $category->parentCategory->category_name.' / ' : '' }}{{ $category->category_name }}</option>@endforeach</select></label>
...
<div class="featured-image-field"><label>{{ __('Image description') }}<textarea id="ai-image-prompt" rows="3" maxlength="4000" placeholder="{{ __('Describe an image or leave blank to use your work.') }}"></textarea></label><button type="button" id="generate-featured-image" data-url="{{ route('send-image-gen-prompt') }}">{{ __('Draw image with AI') }}</button><label>{{ __('Featured image') }}<input id="featured-image-upload" type="file" accept="image/jpeg,image/png,image/gif"></label><input type="hidden" name="featured_image"><img id="featured-image-preview" src="{{ $book->getOriginalUrl() }}" alt="{{ __('Featured image') }}" style="max-width:100%" @if(!$book->featured_image) hidden @endif><button type="button" id="remove-featured-image">{{ __('Remove image') }}</button><small>{{ __('Save book details to apply the image.') }}</small></div>
