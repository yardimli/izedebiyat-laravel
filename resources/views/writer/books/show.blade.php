...
<button class="primary">{{ __('Save book details') }}</button></form></section>
        ...
        <section class="writing-pane" aria-label="{{ __('Manuscript editor') }}">
            ...
            <footer class="page-navigation"><span id="word-count">{{ __('0 words') }}</span><button type="button" id="toggle-page-breaks" aria-pressed="false">{{ __('Show page breaks') }}</button><span id="ai-thinking-status" role="status" hidden>{{ __('AI is thinking. Please don’t write anything.') }}</span></footer>
        </section>
...
<dialog id="writing-welcome" data-hidden-for-session="{{ session('writer_welcome_hidden', false) ? '1' : '0' }}" data-preference-url="{{ route('writer.welcome-preference') }}" aria-labelledby="writing-welcome-title" aria-describedby="welcome-model welcome-model-advice">
...
<label class="check"><input type="checkbox" id="welcome-hide-session"> {{ __('Do not show again during this session') }}</label>
