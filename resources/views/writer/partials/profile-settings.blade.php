					...
					<form action="{{ route('backend.sifre-guncelle') }}" method="post"
						...
						<div class="col-12">
							...
							<div class="input-group">
								...
								<button type="button" class="input-group-text profile-password-toggle" aria-label="Şifreyi göster" aria-pressed="false">
                          Göster
                        </button>
							</div>
							<!-- Pswmeter -->
...
@push('scripts')
<script>
passwordStrengthMeter({containerElement:'#pswmeter',passwordInput:'.psw-input',showMessage:true,messageContainer:'#pswmeter-message',messagesList:['Şifrenizi girin','Fazla basit','Basit','Güzel','Harika'],height:8,borderRadius:4,pswMinLength:8});
document.querySelectorAll('.profile-password-toggle').forEach(button=>button.addEventListener('click',()=>{const input=button.parentElement.querySelector('input');const show=input.type==='password';input.type=show?'text':'password';button.textContent=show?'Gizle':'Göster';button.setAttribute('aria-pressed',String(show));button.setAttribute('aria-label',show?'Şifreyi gizle':'Şifreyi göster');}));
</script>
@endpush
