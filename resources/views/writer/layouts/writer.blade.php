<!DOCTYPE html>
<html data-bs-theme="{{ (auth()->user()->theme ?? 'paper') === 'dark' ? 'dark' : 'light' }}" lang="{{ app()->getLocale() }}" data-theme="{{ auth()->user()->theme ?? 'paper' }}">
<head>
...
@stack('styles')
@hasSection('portal')<link rel="stylesheet" href="{{ asset('css/writer-portal.css') }}?v={{ filemtime(public_path('css/writer-portal.css')) }}">@endif
</head>
<body class="{{ $__env->hasSection('portal') ? 'writer-portal' : '' }}" data-app-name="{{ config('app.name') }}" data-user="{{ auth()->id() }}">
<header class="site-header">
    ...
    <nav aria-label="{{ __('Main navigation') }}">
        ...
        <a class="nav-icon" href="{{ route('backend.following') }}" aria-label="Favorilerim" title="Favorilerim"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" aria-hidden="true"><path d="m12 3 2.8 5.7 6.2.9-4.5 4.4 1.1 6.2-5.6-3-5.6 3 1.1-6.2L3 9.6l6.2-.9Z"/></svg></a>
        <a class="nav-icon" href="{{ route('chat') }}" aria-label="Sohbet" title="Sohbet"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" aria-hidden="true"><path d="M21 11a8 8 0 0 1-8 8H7l-5 3 2-6a8 8 0 1 1 17-5Z"/></svg></a>
        ...
        @if(auth()->user()->isAdmin()) @include('writer.partials.admin-menu') @endif
        @endunless
        @endauth
...
@if(session('success'))<div class="server-errors" role="status">{{ session('success') }}</div>@endif
@yield('content')
@hasSection('portal')<script src="{{ asset('assets/vendor/bootstrap/dist/js/bootstrap.bundle.min.js') }}"></script>@endif
@stack('scripts')
</body></html>
