                ...
                <div class="mb-5">
                    ...
                    <h2 class="entry-subtitle mb-3 fst-italic">
                        @if ($bookReview->bookAuthor)
                            <a
                                href="{{ route('frontend.book-reviews.author', $bookReview->bookAuthor->slug) }}">{{ $bookReview->display_author }}</a>
                        @else
                ...
                <div class="row">
                    ...
                    <div class="col-md-4 col-xl-4">
                        <figure class="image zoom mb-4 text-center">
                            <img src="{{ $bookReview->cover_image ? asset($bookReview->cover_image) : asset('images/no-image.png') }}"
                                style="max-width: 300px; width: 100%;" alt="Cover Image">
                        </figure>
                        ...
                        <div class="book-details mb-4">
                            ...
                            @if ($bookReview->publication_date)
                                <p><strong>Yayın Tarihi:</strong>
                                    {{ \Carbon\Carbon::parse($bookReview->publication_date)->format('d F Y') }}</p>
                            @endif
                        </div>

                        @if ($bookReview->buy_url)
                            <div class="text-center">
                                <a href="{{ $bookReview->buy_url }}" class="btn btn-primary" target="_blank"
                            ...
                            @if ($bookReview->categories->isNotEmpty())
                                <div class="tags-wrap heading mb-2">
                                    ...
                                    @foreach ($bookReview->categories as $category)
                                        <a
                                            href="{{ route('frontend.book-reviews.show-by-category', $category->slug) }}">{{ $category->name }}</a>
                                    @endforeach
                                </div>
                            @endif
                            @if ($bookReview->tags->isNotEmpty())
                                <div class="tags-wrap heading">
                                    ...
                                    @foreach ($bookReview->tags as $tag)
                                        <a
                                            href="{{ route('frontend.book-reviews.show-by-tag', $tag->slug) }}">{{ $tag->name }}</a>
                                    @endforeach
                                </div>
                            @endif
                        </div>
