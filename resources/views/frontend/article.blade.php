...
@section('content')
    ...
    margin-bottom: 0px;">
                        ...
                        <span class="middotDivider"></span>
                        <span
                            class="readingTime">{{ \App\Helpers\MyHelper::estimatedReadingTime($article->main_text) }}</span>
                        <span class="middotDivider"></span>
                        <span>{{ \App\Helpers\MyHelper::timeElapsedString($article->created_at) }}</span>
                    </div>

                    <div class="entry-meta align-items-center divider pb-2"
    ...
    margin-bottom: 10px;">
                        <button id="clap" class="clap" data-article-id="{{ $article->id }}">
                            ...
                            <span id="clap--count-total"
                                class="clap--count-total">{{ $article->claps()->sum('count') }}</span>
                        </button>
                        ...
                        <div class="favorite-btn d-inline-block" data-article-id="{{ $article->id }}"
                            ...
                            {!! Auth::check() && $article->favorites->contains('follower_id', Auth::id())
            ...
            @include('partials.related-posts', [
                'sameUserAndCategory' => $sameUserAndCategory,
                'sameUserAndMainCategory' => $sameUserAndMainCategory,
                'otherUserArticles' => $otherUserArticles,
            ])
        </div>
    </main>
        ...
        <div class="comment-panel-content">
            <div class="comment-form mb-4">
                ...
                <button class="btn btn-secondary btn-sm mt-2"
                    onclick="submitComment()">{{ __('default.Submit') }}</button>
            </div>
    ...
    <script>
        ...
        function createCommentHTML(comment) {
            ...
            if (comment.user_id === 0) {
                ...
                userNameHTML =
                    `<div class="comment-author"><a href="/yazar/${comment.user.slug}">${comment.user.name}</a></div>`;
            }
        ...
        $(window).scroll(function() {
            ...
            if (scrollwin >= ($('.entry-main-content').offset().top - 100)) {
                if (scrollwin <= (($('.entry-main-content').offset().top - 100) + (articleheight + 100))) {
                    $('.bar-long').css('width', ((scrollwin - ($('.entry-main-content').offset().top - 100)) / (
                        articleheight + 100)) * 100 + "%");
                } else {
