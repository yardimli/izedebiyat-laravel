        ...
        <div class="container mt-5" style="min-height: calc(88vh);">
            ...
            @if ($deletedReads !== null)
                <div class="alert alert-success alert-dismissible fade show" role="alert">
                    {{ number_format($deletedReads) }} suspicious read records removed. Affected article read counts were
                    recalculated.
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                </div>
            @endif
            ...
            <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-4">
                ...
                <form action="{{ route('admin.read-cleanup.index') }}" method="GET" class="d-flex flex-wrap gap-2">
                    <div class="input-group input-group-sm" style="width: 190px;">
                        ...
                        <input type="number" min="1" max="100000" class="form-control" name="threshold"
                            value="{{ $filters['threshold'] }}">
                    </div>
                    <div class="input-group input-group-sm" style="width: 190px;">
                        <span class="input-group-text">Hours</span>
                        <input type="number" min="0" max="720" class="form-control" name="window_hours"
                            ...
                            <table class="table table-hover align-middle">
                                ...
                                <tbody>
                                    @foreach ($changes as $change)
                                        <tr>
                                            <td>
                                                <a href="{{ route('article', $change->article->slug) }}"
                                                    target="_blank">{{ strip_tags($change->article->title) }}</a>
                                            </td>
