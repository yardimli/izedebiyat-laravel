                                    ...
                                    @foreach ($requests as $request)
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <span
                                                    class="badge bg-{{ $request->status === 'pending' ? 'warning' : ($request->status === 'approved' ? 'success' : ($request->status === 'approved_deleted' ? 'dark' : 'danger')) }}">
                                                    {{ ucfirst($request->status) }}
                                                </span>
                                            </td>
                                            <td>{{ $request->created_at->format('d M Y H:i') }}</td>
                                            <td>
                                                <a href="{{ route('admin.account-recovery.show', $request->id) }}"
                                                    class="btn btn-sm btn-info">Detayları Gör</a>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
