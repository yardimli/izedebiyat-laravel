                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            @if ($request->profile_url)
                                <p><strong>Belirtilen Profil Adresi:</strong> <a href="{{ $request->profile_url }}"
                                        target="_blank">{{ $request->profile_url }}</a></p>
                            @endif
                            ...
                            @endif
                            <p><strong>Durum:</strong> <span
                                    class="badge bg-{{ $request->status === 'pending' ? 'warning' : ($request->status === 'approved' ? 'success' : ($request->status === 'approved_deleted' ? 'dark' : 'danger')) }}">{{ ucfirst($request->status) }}</span>
                            </p>
                    ...
                    <div class="card mt-4">
                        ...
                        <div class="card-body">
                            <a href="{{ Storage::url($request->id_document_path) }}" target="_blank">
                                <img src="{{ Storage::url($request->id_document_path) }}" class="img-fluid"
                                    alt="ID Document">
                            </a>
                        </div>
                    </div>
                    ...
                    @if ($request->status === 'pending')
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                <form action="{{ route('admin.account-recovery.show', $request->id) }}" method="GET"
                                    class="mb-3">
                                    <div class="input-group">
                                        <input type="text" name="search" class="form-control"
                                            placeholder="İsim veya e-posta ile ara..." value="{{ $searchQuery ?? '' }}">
                                ...
                                @if (isset($searchQuery))
                                    ...
                                    @else
                                        <form action="{{ route('admin.account-recovery.approve', $request->id) }}"
                                            method="POST">
                                            @csrf
                                            <div class="mb-3">
                                            ...
                                            <button type="submit"
                                                class="btn btn-{{ $request->delete_account ? 'danger' : 'success' }}">
                                                @if ($request->delete_account)
                    ...
                    @else
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                @if ($request->user)
                                    <p><strong>İlişkili Kullanıcı:</strong> <a
                                            href="{{ route('user', $request->user->slug) }}"
                                            target="_blank">{{ $request->user->name }}</a></p>
                                @endif
