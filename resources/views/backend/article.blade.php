                ...
                <div class="col-12 col-xl-8 col-lg-8 mx-auto">
                    ...
                    <form id="articleForm" method="POST"
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" id="article_alan" name="main_text" rows="3">{!! isset($article) ? $article->main_text : old('main_text') !!} </textarea>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" id="subheading" name="subheading" rows="3">{!! isset($article) ? $article->subheading : old('subheading') !!}</textarea>
                        </div>
                        ...
                        <div class="mb-3">
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" id="is_published" name="is_published"
                                    value="1" {{ isset($article) && $article->is_published ? 'checked' : '' }}>
                        ...
                        <div class="card-body">
                            ...
                            @else
                                <p class="text-muted text-center">
                                    {{ __('default.Comments will be available after creating the article') }}</p>
                            @endif
                        </div>
                    </div>
                ...
                $('.delete-upload-image').on('click', function() {
                    if (confirm('{{ __('default.Are you sure you want to delete this image?') }}')) {
                        ...
                        $.ajax({
                            ...
                            success: function() {
                                showNotification(
                                    '{{ __('default.Image deleted successfully') }}',
                                    'success');
                            },
                            error: function() {
            ...
            @if (isset($article))
                $.get('{{ route('comments.index', $article) }}', function(comments) {
                    ...
                    if (comments.length === 0) {
                        container.html(
                            '<p class="text-center text-muted">{{ __('default.No comments yet') }}</p>');
                        return;
                    }
    ...
    <div class="mt-2">${comment.content}</div>
        ...
        $(document).ready(function() {
                ...
                if (confirm('{{ __('default.Are you sure you want to delete this comment?') }}')) {
                    $.ajax({
                        ...
                        success: function() {
                            ...
                            showNotification(
                                '{{ __('default.Comment deleted successfully') }}',
                                'success');
                        },
                        error: function() {
                            showNotification('{{ __('default.Error deleting comment') }}',
                                'error');
                        }
                    });
                }
            ...
            const easyMDE = new EasyMDE({
                ...
                lineWrapping: true,
                status: ["lines", "words"],
                toolbar: ["bold", "italic", "heading", "quote", "horizontal-rule", "|", "unordered-list",
        ...
        `);
            ...
            $('#generateImageBtn').on('click', function() {
                ...
                $(this).prop('disabled', true).html(
                    '<span class="spinner-border spinner-border-sm"></span> {{ __('default.Generating...') }}'
                );
                ...
                $.ajax({
                    ...
                    success: function(result) {
                        if (result.success) {
                            ...
                            $('#generatedImage').attr('src', '/storage/ai-images/large/' +
                                result.image_large_filename);
            ...
            $(document).on('click', '.delete-generated-image', function() {
                if (confirm('{{ __('default.Are you sure you want to delete this generated image?') }}')) {
                    ...
                    $.ajax({
                        ...
                        success: function() {
                            showNotification(
                                '{{ __('default.Generated image deleted successfully') }}',
                                'success');
                        },
                        error: function() {
                            showNotification(
                                '{{ __('default.Error deleting generated image') }}');
                        }
                    });
                }
            });
            ...
            $('#uploadImageForm').submit(function(e) {
                ...
                $.ajax({
                    ...
                    success: function() {
                        ...
                        showNotification('{{ __('default.Image uploaded successfully') }}',
                            'success');
                        $('#imageModal').modal('show');
                    },
                    error: function() {
            ...
            kategoriSelect.find('option').each(function() {
                if (!$(this).prop('disabled')) {
                    ...
                    if (typeof $(this).data('main-category') !== 'undefined') {
                        $(this).data('short-text', $(this).data('main-category') + ' - ' + $(this).text()
                            .trim());
                    } else {
            ...
            $('#generateCategory').on('click', function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        showNotification(
                            '{{ __('default.Category generated successfully.') }}',
                            'success');
                    },
                    error: function() {
                        showNotification('{{ __('default.Error generating category:') }}',
                            'error');
                    },
                    complete: function() {
                        $('#generateCategory').prop('disabled', false).html(
            ...
            $('#generateDescription').on('click', function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        showNotification(
                            '{{ __('default.Description generated successfully.') }}',
                            'success');
                    },
                    error: function() {
                        showNotification('{{ __('default.Error generating description:') }}',
                            'error');
                    },
                    complete: function() {
                        $('#generateDescription').prop('disabled', false).html(
                            'Yapay Zeka ile Oluştur');
                    }
                });
            });
            ...
            $('#generateKeywords').on('click', function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        showNotification(
                            '{{ __('default.Keywords generated successfully.') }}',
                            'success');
                    },
                    error: function() {
                        showNotification('{{ __('default.Error generating keywords:') }}',
                            'error');
                    },
                    complete: function() {
                        $('#generateKeywords').prop('disabled', false).html(
                            'Yapay Zeka ile Oluştur');
                    }
                });
            });
