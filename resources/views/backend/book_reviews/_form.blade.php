        ...
        @isset($authors)
            @foreach ($authors as $author)
                <option value="{{ $author->id }}"
                    {{ old('book_author_id', $bookReview->book_author_id ?? '') == $author->id ? 'selected' : '' }}>
                    {{ $author->name }}
                </option>
            @endforeach
        @endisset
    </select>
    <small class="form-text text-muted">Eğer yazar listede yoksa, aşağıdaki alana manuel olarak yazabilirsiniz. Yeni
        yazarları <a href="{{ route('book-authors.index') }}" target="_blank">Yazar Yönetimi</a> sayfasından
...
<div class="mb-3">
    ...
    <textarea class="form-control" id="review_content_textarea" name="review_content" rows="15">{{ old('review_content', $bookReview->review_content ?? '') }}</textarea>
</div>
...
<div class="mb-3">
    ...
    @if (isset($bookReview) && $bookReview->cover_image)
        <div class="mt-2">
            ...
            <img src="{{ $bookReview->cover_image }}" alt="Cover Image" class="img-fluid mt-1"
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            var categoriesTagify = new Tagify(categoriesInput, {
                ...
                dropdown: {
                    enabled: 0
                } // No dropdown suggestions from a whitelist
            });
