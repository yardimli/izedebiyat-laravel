window.embedComments = function (div, page, title, embed) {
    var href = window.location.href,
        hash = window.location.hash,
        src = (embed || "embed-comments.php") + "?";

    if (page) src += "&page=" + page;

    if (title) src += "&page_title=" + title;

    src += "&_parent_url=";

    if (hash.indexOf("#comment-") > -1) {
        src += href.substr(0, href.indexOf("#")) + "#comment-" + hash.substr(9, hash.length);
    } else {
        src += href;
    }

    var $iframe = $("<iframe/>", { frameborder: 0, src: src }).css({
        width: "100%",
        minHeight: 210,
        border: "none",
    });
...
window.embedCommentsCallback = function (id, scroll) {
    window.location.hash = "comment-" + id;
    $("body").animate({ scrollTop: scroll + 50 }, 10);
};

window.embedCommentsModalCallback = function (target) {
    $(target).modal("show");
};
...
!(function () {
    ...
    function f() {
        null !== v &&
            (window.scrollTo(v.x, v.y), c(" Set position: " + v.x + "," + v.y), (v = null));
    }
    function g(a) {
        function b() {
