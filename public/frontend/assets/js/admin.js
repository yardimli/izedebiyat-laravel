jQuery(function ($) {
    ...
    EasyLogin.admin.usersDT = function () {
            ...
            columns: [
                ...
                null,
                {
                    ...
                    render: function (d, t, r) {
                        ...
                        return (
                            '<a href="?page=user-edit&id=' +
                            r[0] +
                            '" title="' +
                            trans("edit_user") +
                            '"><span class="glyphicon glyphicon-edit"></span></a> ' +
                            '<a href="?page=message-reply&id=' +
                            r[0] +
                            '" title="' +
                            trans("send_message") +
                            '"><span class="glyphicon glyphicon-share-alt"></span></a> ' +
                            "<a href=\"javascript:EasyLogin.admin.composeEmail('" +
                            r[2] +
                            '\')" title="' +
                            trans("send_email") +
                            '"><span class="glyphicon glyphicon-envelope"></span></a> ' +
                            '<a href="javascript:EasyLogin.admin.deleteUser(' +
                            r[0] +
                            ", '" +
                            (r[1] || r[2]) +
                            '\')" title="' +
                            ...
                            ' <a href="?page=user-login&id=' +
                            r[0] +
                            '" title="Log in as this user" target="_blank"><span class="glyphicon glyphicon-log-in"></span></a>'
                        );
        ...
        */
        ...
        $("#users_length").before(
            '<button type="submit" class="btn btn-danger btn-sm delete-bulk" disabled>' +
                trans("delete") +
                "</button>",
        );
                ...
                null,
                {
                    render: function (d, t, r) {
                        return (
                            ...
                            trans("reply") +
                            '"><span class="glyphicon glyphicon-share-alt"></span></a> ' +
                            '<a href="javascript:EasyLogin.admin.deleteConversation(' +
                            r[5] +
                            ", '" +
                            r[2] +
                            '\')" title="' +
                            trans("delete_conversation") +
                            '"><span class="glyphicon glyphicon-trash"></span></a>'
                        );
                    },
                },
    ...
    EasyLogin.admin.searchContact = function () {
        ...
        div.on("input", ".search", function (e) {
            ...
            $.get(
                EasyLogin.options.ajaxUrl,
                { action: "searchContact", admin: true, user: value },
                function (response) {
            ...
            columns: [
                {
                    ...
                    render: function (data) {
                        return (
                            '<input type="checkbox" name="comments[]" value="' +
                            data +
                            '" class="chb-select">'
                        );
                    },
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        ...
                        if (row[9] === "0") {
                            html +=
                                '<a href="javascript:EasyLogin.admin.commentAction(' +
                                id +
                                ", 'approve')\" title=\"" +
                                trans("approve_comment") +
                        ...
                        if (row[9] === "1") {
                            html +=
                                ...
                                id +
                                ", 'unapprove')\" title=\"" +
                                trans("unapprove_comment") +
                                '"><span class="glyphicon glyphicon-remove"></span></a> ';
                        }
                        if (status != "2") {
                            html +=
                                '<a href="javascript:EasyLogin.admin.commentReply(' +
                                id +
                                ", '" +
                                row[6] +
                                '\')" title="' +
        ...
        commentsDataTable.on("draw", function () {
            ...
            $("#comments_length").before(
                '<div class="bulk-action"><select name="bulk_action" class="form-control input-sm">' +
                    ...
                    "</option>" +
                    (status != "0" && status != "2"
                        ? '<option value="unapprove">' + trans("unapprove") + "</option>"
                        : "") +
                    (status != "2" && status != "1"
                    ...
                    (status === "2"
                        ? '<option value="approve">' + trans("restore") + "</option>"
                        : "") +
                    (status === "2"
                        ? '<option value="delete">' + trans("delete_permanently") + "</option>"
                        : "") +
                    '</select> <button type="submit" class="btn btn-default btn-sm">' +
                    trans("apply") +
                    "</button></div>",
            );
        });
    ...
    $(".table-dt").on("click", ".select-all", function (e) {
        ...
        $(".delete-bulk").prop("disabled", $(".chb-select:checked").length ? false : true);
    });
    ...
    $(".table-dt").on("click", ".chb-select", function (e) {
        ...
        $(e.delegateTarget)
            .find(".select-all")
            .prop("checked", $(e.delegateTarget).find(".chb-select").length == checked);
...
(function (e, t, n) {
    ...
    e.fn.dropdownHover = function (n) {
        ...
        return this.each(function () {
            ...
            var i = e(this),
                s = i.parent(),
                o = { delay: 100, instantlyCloseOthers: !0 },
                u = {
